...
RUNTIME_SCHEMAS = {
    ...
    'ue_create_ta_asset': ({'destination': {'type': 'string'}, 'kind': {'type': 'string', 'enum': ['level_sequence']}, 'source_asset': {'type': 'string'}, 'expected_revision': {'type': 'string'}}, ['destination'], 'Core profile can create a LevelSequence in memory, or duplicate an existing supported /Game asset with its current revision. No save, no overwrite.'),
    'ue_inspect_ta_asset': ({'asset_path': {'type': 'string'}, 'view': {'type': 'string'}, 'name': {'type': 'string'}, 'lod': {'type': 'integer', 'minimum': 0}, 'offset': {'type': 'integer', 'minimum': 0, 'maximum': 2147483647}, 'limit': {'type': 'integer', 'minimum': 1, 'maximum': 500}}, ['asset_path'], 'Inspect a core-compatible extended asset and get its dedicated revision. BlendSpace is read-only; sequence views support notifies/curves; Montage supports sections/slots/notifies/curves; Mesh supports lods or morph_deltas (name/lod required); Physics supports bodies/constraints; LevelSequence supports bindings/sections. Skeleton returns revision only. Nested reflected arrays are capped at 100, morph deltas are independently paginated.'),
    ...
    'ue_save_ta_asset': (COMMON_WRITE, ['asset_path', 'expected_revision'], 'Back up and save the current core-compatible extended asset, including user edits. Use ue_inspect_ta_asset revision. This does not prove playback correctness.'),
